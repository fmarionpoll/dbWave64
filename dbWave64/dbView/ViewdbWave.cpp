...
void ViewdbWave::OnInitialUpdate()
{
	...
	m_data_list_ctrl_.SetExtendedStyle(m_data_list_ctrl_.GetExtendedStyle() | LVS_EX_FULLROWSELECT | LVS_EX_ONECLICKACTIVATE | LVS_EX_SUBITEMIMAGES);
	...
	for (int i = 0; i < N_COLUMNS2; ++i)
	{
		...
		const int w = ReadRegistryInt(_T("dbView2\\DataListCtrl2"), key, -1);
	...
	m_data_list_ctrl_.init(provider, settings, data_renderer, spike_renderer, have_widths ? &m_columns_width_ : nullptr);
...
void ViewdbWave::OnUpdate(CView* p_sender, const LPARAM l_hint, CObject* p_hint)
{
	...
	case HINT_GET_SELECTED_RECORDS:
	{
		...
		const int selected_count = static_cast<int>(m_data_list_ctrl_.GetSelectedCount());
		if (selected_count > 0)
		{
			...
			{
				n_item = m_data_list_ctrl_.GetNextItem(n_item, LVNI_SELECTED);
				ASSERT(n_item != -1);
	...
	case HINT_SET_SELECTED_RECORDS:
	{
		...
		int item = -1;
		item = m_data_list_ctrl_.GetNextItem(item, LVNI_SELECTED);
		while (item != -1)
		{
			m_data_list_ctrl_.SetItemState(item, 0, LVIS_SELECTED);
			item = m_data_list_ctrl_.GetNextItem(item, LVNI_SELECTED);
		}
		...
		if (u_selected_count > 0)
		{
			for (UINT i = 0; i < u_selected_count; i++)
			{
				...
				m_data_list_ctrl_.SetItemState(item, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
			}
		}
		m_data_list_ctrl_.EnsureVisible(item, FALSE);
	}
	break;
...
void ViewdbWave::delete_records()
{
	...
	auto pos = m_data_list_ctrl_.GetFirstSelectedItemPosition();
	if (pos == nullptr)
	{
	...
	while (pos)
	{
		const auto n_item = m_data_list_ctrl_.GetNextSelectedItem(pos);
...
void ViewdbWave::save_controls_state()
{
	...
	state.b_display_file_name = m_data_list_ctrl_.get_display_file_name();
	state.b_filter_dat = m_data_list_ctrl_.get_transform_mode() == DataTransform::MedianFilter;
	state.b_set_time_span = m_data_list_ctrl_.get_timespan_adjust_mode();
	state.t_first = m_time_first_;
	state.t_last = m_time_last_;
	state.b_set_mv_span = m_data_list_ctrl_.get_amplitude_adjust_mode();
...
void ViewdbWave::on_hdn_end_track_list_ctrl(NMHDR* p_nmhdr, LRESULT* p_result)
{
	...
	*p_result = 0;
	...
	{
		CString key; key.Format(_T("col_%d"), i);
		WriteRegistryInt(_T("dbView2\\DataListCtrl2"), key, m_data_list_ctrl_.GetColumnWidth(i));
	}
}
...
void ViewdbWave::on_bn_clicked_data()
{
	...
	m_data_list_ctrl_.set_display_mode(DisplayMode::Data);
...
void ViewdbWave::on_bn_clicked_display_spikes()
{
	...
	m_data_list_ctrl_.set_display_mode(DisplayMode::Spikes);
	const BOOL all_classes = static_cast<CButton*>(GetDlgItem(IDC_RADIOALLCLASSES))->GetCheck();
	m_data_list_ctrl_.set_spike_plot_mode(all_classes ? SpikePlotMode::AllClasses : SpikePlotMode::OneClass, m_spike_class_);
	m_data_list_ctrl_.refresh_display();
}
...
void ViewdbWave::on_bn_clicked_display_nothing()
{
	...
	m_data_list_ctrl_.set_display_mode(DisplayMode::None);
	m_data_list_ctrl_.refresh_display();
}
...
void ViewdbWave::on_click_median_filter()
{
	...
	m_data_list_ctrl_.set_transform_mode(on ? DataTransform::MedianFilter : DataTransform::None);
	m_data_list_ctrl_.refresh_display();
}
