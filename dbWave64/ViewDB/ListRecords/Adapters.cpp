...
void ChartDataRendererAdapter::renderBitmap(const DisplaySettings& settings, const RowMeta& meta, CBitmap& out_bitmap) 
{
    ...
    mem_dc.SelectObject(&out_bitmap);
    mem_dc.FillSolidRect(0, 0, settings.image_width, settings.image_height, RGB(255, 255, 255));
    // Minimal placeholder rendering for unit tests (no external dependencies)
    CRect r(0,0,settings.image_width, settings.image_height);
    mem_dc.DrawEdge(&r, EDGE_SUNKEN, BF_RECT);
#endif
}
...
void ChartSpikeRendererAdapter::renderBitmap(const DisplaySettings& settings, const RowMeta& meta, CBitmap& out_bitmap) 
{
    ...
    out_bitmap.CreateBitmap(settings.image_width, settings.image_height,
        screen_dc.GetDeviceCaps(PLANES), screen_dc.GetDeviceCaps(BITSPIXEL), nullptr);
    mem_dc.SelectObject(&out_bitmap);
    mem_dc.FillSolidRect(0, 0, settings.image_width, settings.image_height, RGB(230, 230, 230));
    // Minimal placeholder stripe for unit tests
    CPen pen(PS_SOLID, 1, RGB(120,120,120));
    const auto oldp = mem_dc.SelectObject(&pen);
    mem_dc.MoveTo(0, settings.image_height/2);
    mem_dc.LineTo(settings.image_width, settings.image_height/2);
    if (oldp) mem_dc.SelectObject(oldp);
#endif
}